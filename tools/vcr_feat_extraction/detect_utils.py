...
def detect_objects_on_single_image(model, transforms, cv2_img, box_list: BoxList):
    ...
    boxlist_input = box_list.resize(size=(img_input.shape[-2], img_input.shape[-1]))
    ...
    if isinstance(model, SceneParser):
        ...
        rt_relation_list = [
            ...
            for relation, predicate, score in zip(
                relations, predicates, relation_scores
            )
        ]
